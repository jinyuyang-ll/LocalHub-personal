...
public class OrderPaymentService {
    ...
    public Result pay(Long orderId) {
        ...
        if (order.getStatus() == null || order.getStatus() != UNPAID) return Result.fail("订单当前状态不可支付");
        ...
        int updated = orderMapper.update(null, new UpdateWrapper<VoucherOrder>()
                ...
                .set("status", PAID).set("pay_time", paidAt));
        if (updated != 1) return Result.fail("订单支付失败，请刷新后重试");
        order.setStatus(PAID); order.setPayTime(paidAt);
